...
export type TreeSitterParser = any
export type TreeSitterQuery = any
export type TreeSitterLangs = { [lang: string]: any }
...
export const initializeArParser = (Parser: TreeSitterParser, Query: TreeSitterQuery, langs: TreeSitterLangs): ArParser => {
  ...
  const getFishSegs = (dict, lang) => (str: string, option?: { all: boolean }): [Pair[], string[], string] => {
    ...
    const q = new Query(langs.typescript, query);

    const importQuery = new Query(langs.typescript, "(import_statement)@import")
  ...
  const makeQuery = (inputContent: string = fs.readFileSync('./index.ts', 'utf-8'), filePath = './index.ts'): ArST_withMetaInfo => {
  ...
  const parseAndMakeArST = (fishDictionary: FISH_DICT) => (content: string, nestedIndex: number[] = [], LEVEL_TO_STOP = 4): ArST => {
    ...
    const parser = new Parser();
    parser.setLanguage(langs.typescript);
  ...
  return { makeQuery, idOfArST, flattenArST }
