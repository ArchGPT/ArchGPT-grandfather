...
export function applyMakeQueryToDir(
  folderPath: string,
  parser: ArParser,
  ...
  return _.flatten(
    files.map((file) => {
      ...
      const gitIgnoredContentHere =
        (fs.existsSync(path.join(folderPath, ".gitignore")) &&
          fs.readFileSync(path.join(folderPath, ".gitignore"), "utf8")) ||
        "";
      const currentIgnoredFiles: string[] =
        parseGI(gitIgnoredContentHere)?.patterns || [];

      const applyToCurrentFolder = currentIgnoredFiles.filter(
        (a) => "/" === a[0]
      );
      ...
      if (
        _.includes(
          [
            ...applyToCurrentFolder.map((a) => a.slice(1).replace(/\/$/, "")),
            ".git",
            ".archgpt",
          ],
          file
      ...
      const allIgnored = [
        ...gitIgnoredFiles,
        ..._.without(currentIgnoredFiles, ...applyToCurrentFolder).map((a) =>
          a.replaceAll(".", "\\.")
        ),
      ];
      ...
      if (stats.isDirectory()) {
        ...
        if (
          [".js", ".ts", ".tsx", ".jsx"].some((a) =>
            path.extname(filePath).endsWith(a)
          )
        ) {
...
export const initArchGPT = (option: ArchGPTOption = {}): ARCH_GPT => {
  ...
  return {
    ...
    initParser: initializeArParser,
    initDB: async (
      folder: string,
      option?: { fromScratch: boolean }
    ): Promise<{ db: Connection; tables: Table<EmbeddingEntry>[] }> => {
    ...
    searchFiles: async (
      query: string,
      options: { printNamesOnly?: boolean } = {}
    ): Promise<ArST_withMetaInfo[]> => {
      const r = searchFiles(hs, option.converDBPathToMatchF_Path)(
        tables[0],
        query,
        options
      );
    ...
    runPrompt: async (
      purpose: string,
      _config: PromptConfig | PromptConfigToUseTemplate
    ): Promise<string> => {
      ...
      const config: PromptConfig = toLoad
        ? defaultTemplates![toLoad](
            (_config as PromptConfigToUseTemplate).promptTemplateData
          )
        : (_config as PromptConfig);

      console.log("config.llm", config.llm);

      if (!streamInterpreter && !_config.filePath) {
        console.log(
          "config.intrepretStream is not defined, we will not stream"
        );
      ...
      const interpreter: CallbackObj = {
        take: (str: string) => {
          ...
          if (streamInterpreter) streamInterpreter.take?.(str);
          if (_config.savedToHistory) {
        ...
        shouldContinue: () => {
          if (streamInterpreter) return streamInterpreter.shouldContinue();
          return true;
        },
      };

      if (_.includes(localLLMs, config.llm)) {
        const stringPrompt = config.promptInput as string;
        await runViaOllama(
          {
            llm: config.llm,
            prompt: stringPrompt,
        ...
        const fullPrompt =
          config.promptInput as Array<ChatCompletionMessageParam>;
...
const createFile = (description: string, ArST: ArST_withMetaInfo[]) => [
  `Given a file or code snippet, you will implement a feature based on an english description.`,
  `Here is the content of \`${ArST[0].filePath}\`:
...
const editFile = (description: string, ArST: ArST_withMetaInfo[]) => [
  `Given a file or code snippet, you will implement a feature based on an english description.`,
  `Here is the content of \`${ArST[0].filePath}\`:
